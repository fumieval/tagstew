...
module Text.HTML.TagStew.Entity(
    lookupEntity
    ) where
...
lookupEntity :: B.ByteString -> Maybe B.ByteString
lookupEntity bs
  | B.head bs == '#' = lookupNumericEntity (B.tail bs)
  | otherwise = lookupNamedEntity bs
...
lookupNumericEntity :: B.ByteString -> Maybe B.ByteString
lookupNumericEntity = fmap (encodeUtf8 . T.pack) . f . B.unpack
        -- entity = '&#' [0-9]+ ';' | '&#x' [0-9a-fA-F]+ ';'
    where
        f (x:xs) | x `elem` ("xX" :: String) = g [('0','9'),('a','f'),('A','F')] readHex xs
        f xs = g [('0','9')] reads xs
...
lookupNamedEntity :: B.ByteString -> Maybe B.ByteString
lookupNamedEntity x = SB.fromShort <$> HM.lookup (SB.toShort x) (getCompact htmlEntities)
...
htmlEntities :: Compact (HM.HashMap SB.ShortByteString SB.ShortByteString)
htmlEntities = unsafePerformIO $ compact $ HM.fromList $ let a * b = (a, SB.toShort $ encodeUtf8 b) in
    ["Aacute" * "\x00C1"
    ,"aacute" * "\x00E1"
    ,"Aacute;" * "\x00C1"
    ,"aacute;" * "\x00E1"
